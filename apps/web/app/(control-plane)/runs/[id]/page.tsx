...
export default async function RunDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ scenario?: string }> }) {
  ...
  return <div className="grid gap-[clamp(2rem,6vw,5rem)]"><PageHeading index="04 / detail" title="Run detail" summary="Review the safe product projection, then follow stable Eve, Vercel, or OpenTelemetry links for deeper execution evidence." /><RunDetail result={result} /></div>;
}
