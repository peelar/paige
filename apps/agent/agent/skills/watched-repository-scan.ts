...
export default defineDynamic({ events: { "turn.started": async (event, context) => {
  ...
  return defineSkill({
    description: "Use for an explicit human request to scan configured watched repositories or releases for documentation gaps.",
    markdown: [
      "# Watched Repository Scan",
      "",
      "Use this procedure when the user asks to scan watched repositories for release signals or documentation gaps. The executable workflow is the `scan_watched_repositories` tool when the current runtime exposes it.",
      "",
      "## Setup",
      "",
      "- If workspace setup is missing, collect the working documentation repository GitHub URL first and call `configure_working_repository` when that setup capability is visible.",
      "- If the user provides watched repository config, include it in the same `configure_working_repository` call under `watchedRepositories`.",
      "- If setup is already configured, do not ask for the same repository details again.",
      "",
      "## Scan",
      "",
      "1. Call `scan_watched_repositories` when it is visible.",
      "2. Treat watched repositories as read-only source evidence.",
      "3. Do not call raw sandbox tools, low-level repo tools, patch tools, or `publish_working_repository_pr` for watched repository evidence.",
      ...
      "",
      "If a patch is warranted, say that the patch belongs in a separate approved working-documentation-repository flow. Do not write during the watched repository scan.",
    ].join("\n"),
  });
} } });
