...
export default defineDynamic({
  events: {
    "step.started": async (event, context) => {
      if (!(await resolveDynamicCapabilities(event, context)).toolNames.includes("configure_working_repository")) return null;
      return defineTool({
        description:
        ...
        async execute(input, ctx) {
          await requireCapabilityToolExecution("configure_working_repository", ctx);
